...
export const CONQUISTAS = [
  ...
  // ===== SOCIAL =====
  { id: "primeiro_amigo", nome: "Laços Formados", desc: "Converse com alguém no WhatsApp pela primeira vez", icone: "🤝", raridade: "comum", categoria: "Social" },
  { id: "popular", nome: "Alma da Festa", desc: "Converse com 5 pessoas diferentes no WhatsApp", icone: "🎉", raridade: "incomum", categoria: "Social" },
  ...
  // ===== CRAFT =====
  { id: "aprendiz_craft", nome: "Aprendiz de Artesão", desc: "Desbloqueie seu primeiro talento de craft", icone: "🔨", raridade: "comum", categoria: "Craft" },
  { id: "primeiro_craft", nome: "Mãos Hábéis", desc: "Crafte seu primeiro item", icone: "⚒️", raridade: "comum", categoria: "Craft" },
  { id: "craft_10", nome: "Artesão Dedicado", desc: "Crafte 10 itens", icone: "🛠️", raridade: "incomum", categoria: "Craft" },
  { id: "craft_50", nome: "Mestre Artesão", desc: "Crafte 50 itens", icone: "🏭", raridade: "raro", categoria: "Craft" },
  { id: "todos_talentos_craft", nome: "Polímata", desc: "Desbloqueie todos os talentos de craft", icone: "📚", raridade: "lendario", categoria: "Craft" },
  { id: "receita_custom", nome: "Inovador", desc: "Crie uma receita de craft personalizada", icone: "📝", raridade: "raro", categoria: "Craft" },
  // ===== ESPECIAIS =====
...
function Conquistas({ userEmail, userNick, isMaster, onClose }) {
  ...
  useEffect(() => {
    ...
    const verificar = async () => {
      ...
      // Craft - Talentos
      const talentosCraft = [
        "Craft de Cibernética",
        "Craft de Equipamentos de Combate",
        "Craft das Gambiarras",
        "Craft de Vestes e Armaduras",
        "Craft de Medicamentos",
        "Craft de Alimentos Consumíveis",
      ];
      const caracteristicas = fichaData.caracteristicas || "";
      const temTalentosCraft = talentosCraft.filter(t => caracteristicas.includes(t));
      if (temTalentosCraft.length >= 1 && !novas["aprendiz_craft"]) { novas["aprendiz_craft"] = { data: Date.now() }; mudou = true; }
      if (temTalentosCraft.length >= 6 && !novas["todos_talentos_craft"]) { novas["todos_talentos_craft"] = { data: Date.now() }; mudou = true; }
      
      // Craft - Contagem de itens craftados
      const itensCraftados = [
        ...(fichaData.equipamentos || []),
        ...(fichaData.vestes || []),
        ...(fichaData.diversos || []),
      ].filter(item => item.origem === "craft");
      if (itensCraftados.length >= 1 && !novas["primeiro_craft"]) { novas["primeiro_craft"] = { data: Date.now() }; mudou = true; }
      if (itensCraftados.length >= 10 && !novas["craft_10"]) { novas["craft_10"] = { data: Date.now() }; mudou = true; }
      if (itensCraftados.length >= 50 && !novas["craft_50"]) { novas["craft_50"] = { data: Date.now() }; mudou = true; }
    ...
    window.addEventListener('desbloquearCraft', handleCraftConquista);
    return () => window.removeEventListener('desbloquearCraft', handleCraftConquista);
