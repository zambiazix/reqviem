...
// ==================== OCTÓGONO DE AVALIAÇÃO ====================
function OctogonoAvaliacao({ valores = {}, onChange, corAura = "#00e0ff", isMaster = false, fichaEmail = null }) {
  ...
  const CATEGORIAS = [
    ...
    "Resiliência à Narrativa",
    "Conhecimento do Sistema/Cenário",
  ...
  const getPonto = (index, valor) => {
    const angulo = (Math.PI * 2 * index) / 8 - Math.PI / 2;
    const raioValor = (valor / raioMax) * raio;
    return {
      x: centro + Math.cos(angulo) * raioValor,
      y: centro + Math.sin(angulo) * raioValor,
    };
  };
  
  const getPontoExterno = (index) => {
    const angulo = (Math.PI * 2 * index) / 8 - Math.PI / 2;
    return {
  ...
  // Posição dos labels FORA do octógono
  const getLabelPosicao = (index) => {
    const angulo = (Math.PI * 2 * index) / 8 - Math.PI / 2;
    const distanciaLabel = raio + 35; // Distância do centro até o label
    return {
      x: centro + Math.cos(angulo) * distanciaLabel,
      y: centro + Math.sin(angulo) * distanciaLabel,
    };
  };
  ...
  const handleMouseMove = (e) => {
    if (arrastando === null || !isMaster) return;
    ...
    const dx = mouseX - centro;
    const dy = mouseY - centro;
    const distancia = Math.sqrt(dx * dx + dy * dy);
    
    // Calcula o valor baseado na distância do centro
    let valor = Math.round((distancia / raio) * raioMax);
    valor = Math.max(0, Math.min(5, valor));
    
    onChange(arrastando, valor);
  };
  ...
  // Gera o path do polígono
  const gerarPath = () => {
    return CATEGORIAS.map((_, index) => {
      const valor = valores[index] || 0;
      const ponto = getPonto(index, valor);
  ...
  return (
    <Box sx={{ 
      ...
      >
        {/* Linhas de grade */}
        {[1, 2, 3, 4, 5].map((nivel) => {
          const pathGrid = CATEGORIAS.map((_, index) => {
            const ponto = getPonto(index, nivel);
            return `${index === 0 ? 'M' : 'L'} ${ponto.x},${ponto.y}`;
          }).join(' ') + ' Z';
        ...
        {CATEGORIAS.map((_, index) => {
          const pontoExterno = getPontoExterno(index);
          return (
            <line
              key={index}
              x1={centro}
            ...
            <g key={index}>
              ...
              <circle
                ...
                style={{ 
                  ...
                  filter: `drop-shadow(0 0 4px ${corAura})`,
                }}
      ...
      {/* Labels FORA do octógono */}
      {CATEGORIAS.map((categoria, index) => {
        const labelPos = getLabelPosicao(index);
        const valor = valores[index] || 0;
        const angulo = (Math.PI * 2 * index) / 8 - Math.PI / 2;
        
        // Ajusta alinhamento baseado na posição
        let textAlign = 'center';
        let transform = 'translate(-50%, -50%)';
        
        if (Math.abs(Math.cos(angulo)) > 0.7) {
          // Lado direito ou esquerdo
          textAlign = Math.cos(angulo) > 0 ? 'left' : 'right';
          transform = Math.cos(angulo) > 0 ? 'translate(5px, -50%)' : 'translate(calc(-100% - 5px), -50%)';
        } else if (Math.abs(Math.sin(angulo)) > 0.7) {
          // Topo ou base
          textAlign = 'center';
        ...
        return (
          ...
          >
            <Typography
              sx={{
                fontSize: '0.55rem',
                color: valor > 0 ? corAura : '#64748b',
                fontWeight: valor > 0 ? 'bold' : 'normal',
                lineHeight: 1.2,
                textShadow: valor > 0 ? `0 0 4px ${corAura}` : 'none',
                userSelect: 'none',
...
function JanelaPerfil({ perfil, onClose, isMaster, todasFichas, xpMap, jogadorPorFicha, ranking, fichasMap }) {
  ...
  useEffect(() => {
    ...
    const ref = doc(db, "avaliacoes_mestre", perfil.fichaEmail);
    const unsub = onSnapshot(ref, (snap) => {
      if (snap.exists()) {
        setAvaliacoesMestre(snap.data().valores || {});
      } else {
        setAvaliacoesMestre({});
      }
    });
  ...
  // 🟢 FUNÇÃO PARA SALVAR AVALIAÇÃO DO MESTRE
  const salvarAvaliacao = async (index, valor) => {
    if (!isMaster || !perfil.fichaEmail) return;
    
    const novosValores = { ...avaliacoesMestre, [index]: valor };
    setAvaliacoesMestre(novosValores);
    
    try {
      await setDoc(doc(db, "avaliacoes_mestre", perfil.fichaEmail), {
        ...
        <Box sx={{ flex: 1, overflowY: "auto", p: 2, "&::-webkit-scrollbar": { width: "4px" }, "&::-webkit-scrollbar-thumb": { background: `${auraCor}44`, borderRadius: "10px" } }}>
          ...
          <Box sx={{ display: "flex", gap: 2, mb: 2, flexWrap: "wrap" }}>
            ...
            <Box sx={{ flex: 1 }}>
              ...
              <Box sx={{ mt: 1, display: "flex", gap: 1, alignItems: "center", minWidth: 0 }}>
                <Box sx={{ display: "flex", flexDirection: "column", gap: 0.3, minWidth: 0, flex: 0 }}>
                  ...
                  {fichaVinculada?.origem && (
                    <Typography variant="caption" sx={{ color: "#4caf50" }}>🏙️ {fichaVinculada.origem}</Typography>
                  )}
                ...
                {/* 🟢 OCTÓGONO DE AVALIAÇÃO DO MESTRE */}
                <Box sx={{ flex: 1, display: 'flex', justifyContent: 'center' }}>
                  <OctogonoAvaliacao
                    valores={avaliacoesMestre}
                    onChange={salvarAvaliacao}
                    corAura={auraCor}
                    isMaster={isMaster}
                    fichaEmail={perfil.fichaEmail}
                  />
                </Box>
              </Box>
            </Box>
...
function PerfilDetalhado({ isMaster = false, visible = false, onClose = () => {}, currentUserEmail = null, fichasMap = {} }) {
  ...
  const getRanking = () => {
    ...
    Object.keys(xpMap).forEach(email => {
      ...
      // 🔴 EXCLUI APENAS O EMAIL DO MESTRE (a conta pessoal dele)
      if (email === "mestre@reqviemrpg.com") return; // Pula a conta do mestre
   ...
   const ranking = useMemo(() => getRanking(), [xpMap, fichasMap]);
  ...
  return (
    <>
      {Object.entries(janelasAbertas).map(([id, perfil]) => (
        <JanelaPerfil key={id} perfil={perfil} onClose={() => fecharPerfil(id)} isMaster={isMaster} todasFichas={fichasMap} xpMap={xpMap} jogadorPorFicha={jogadorPorFicha} ranking={ranking} fichasMap={fichasMap} />
      ))}
      ...
      >
        ...
        >
          <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
            ...
            <Typography variant="subtitle2" sx={{ fontWeight: "bold" }}>{minimizado ? "📚 Wikipédia" : "📚 Wikipédia Reqviem"}</Typography>
          </Box>
          <Box sx={{ display: "flex", gap: 0.5 }}>
            {isMaster && (
