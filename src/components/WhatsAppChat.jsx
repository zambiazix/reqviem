...
import { collection, doc, onSnapshot, addDoc, serverTimestamp, query, orderBy, updateDoc, deleteDoc, setDoc, getDoc, limit } from "firebase/firestore";
...
const MessageBubble = memo(({ 
  ...
  editandoMsg, textoEdicao, editarMensagem, excluirMensagem,
  divisoriaRef, userNick, userEmail
}) => {
  const mostrarDivisoria = ultimaLeitura[chatAberto] && msg.id > ultimaLeitura[chatAberto] && mensagens[index - 1]?.id === ultimaLeitura[chatAberto];
    ...
    <React.Fragment>
      ...
      <Box sx={{ 
        ...
        <Box className="msg-actions" sx={{ 
          ...
          <IconButton size="small" onClick={() => { setEditandoMsg(msg.id); setTextoEdicao(msg.conteudo); }} sx={{ bgcolor: '#1a1a2e', width: 18, height: 18, pointerEvents: 'auto' }}>
            <EditIcon sx={{ fontSize: 10, color: '#ff9800' }} />
          </IconButton>
          <IconButton size="small" onClick={() => excluirMensagem(msg.id)} sx={{ bgcolor: '#1a1a2e', width: 18, height: 18, pointerEvents: 'auto' }}>
...
function WhatsAppChat({ userEmail, userNick, fichasMap, onClose, notificacoesSidebar, setNotificacoesSidebar }) {
  ...
  const formatarHora = useCallback((timestamp) => {
  ...
  const handleFileChange = useCallback(async (e) => {
    ...
    try {
      ...
      setFilePreviews(prev => [...prev, ...urlsValidas]);
    } finally {
  ...
  useEffect(() => {
    const handlePaste = async (e) => {
      ...
      if (imagens.length > 0) {
        setEnviandoImagem(true);
        try {
          const urls = await Promise.all(imagens.map(f => uploadImagem(f)));
          const urlsValidas = urls.filter(url => url !== null);
          setFilePreviews(prev => [...prev, ...urlsValidas]);
        } finally {
          setEnviandoImagem(false);
  ...
  }, [textoMsg, filePreviews, chatAberto, userEmail, enviandoImagem]);

  // 🟢 EDITAR MENSAGEM
  const editarMensagem = useCallback(async (msgId) => {
    ...
    setEditandoMsg(null);
    setTextoEdicao("");
  }, [textoEdicao, userEmail, chatAberto]);

  // 🟢 EXCLUIR MENSAGEM
  const excluirMensagem = useCallback(async (msgId) => {
            ...
            <>
              ...
              <Box sx={{ p: 1, borderTop: "1px solid #334155", bgcolor: "#1a1a2e" }}>
                  ...
                  <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5, mb: 0.5 }}>
                    {filePreviews.map((img, i) => (
                      <Box key={i} sx={{ position: 'relative' }}>
                        <img src={img} loading="lazy" decoding="async" style={{ width: 50, height: 50, borderRadius: 6, objectFit: 'cover' }} />
                ...
                <Box sx={{ display: "flex", gap: 0.5, alignItems: 'flex-end' }}>
                  ...
                  <IconButton size="small" onClick={enviarMensagem} sx={{ color: '#00e0ff' }} disabled={enviandoImagem}>
                    {enviandoImagem ? '⏳' : <SendIcon fontSize="small" />}
                  </IconButton>
                </Box>
              </Box>
            </>
