...
import { doc, deleteDoc, onSnapshot, collection, setDoc, getDoc, getDocs } from "firebase/firestore";
...
export default function HomePage({ user, role, fichasList, selectedFichaEmail, setSelectedFichaEmail, criarContaEJogador }) {
  ...
  const carregarContasUsuarios = async () => {
    try {
      const snap = await getDocs(collection(db, "contas_usuarios"));
      const contas = snap.docs.map(d => ({ id: d.id, ...d.data() }));
      setContasUsuarios(contas);
      setModalContasOpen(true);
    } catch (err) {
  ...
  const alterarStatusConta = async (email, isConvidado) => {
    try {
      ...
      await setDoc(doc(db, "fichas", email), { isConvidado: !isConvidado }, { merge: true });
      setContasUsuarios(prev => prev.map(c => 
        c.email === email ? { ...c, isConvidado: !isConvidado } : c
      ));
      alert(`✅ ${email} agora é ${!isConvidado ? '🎭 Convidado' : '🎮 Jogador'}!`);
    } catch (err) {
  ...
  const fichasFiltradas = useMemo(() => {
    ...
    let pj = todasFichas.filter(fid => (fichasDataMap[fid]?.tipoFicha || "PJ") === "PJ" && fid !== "mestre@reqviemrpg.com" && !fichasDataMap[fid]?.isConvidado);
    let pm = todasFichas.filter(fid => fichasDataMap[fid]?.tipoFicha === "PM" && fid !== "mestre@reqviemrpg.com");
    let convidados = todasFichas.filter(fid => fichasDataMap[fid]?.isConvidado === true && fid !== "mestre@reqviemrpg.com");
    ...
    if (termoBusca.trim()) {
      ...
      pj = pj.filter(filtro);
      pm = pm.filter(filtro);
      convidados = convidados.filter(filtro);
    }
    ...
    pj.sort(ordenar);
    pm.sort(ordenar);
    convidados.sort(ordenar);
    
    return { pj, pm, convidados };
    ...
    <Paper sx={{ 
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1 }}>
        <Typography variant="h6" sx={{ color: "#fff" }}>Fichas</Typography>
          ...
          size="small"
          variant="outlined"
          onClick={carregarContasUsuarios}
          sx={{ 
            color: '#00e0ff',
            borderColor: '#00e0ff',
            '&:hover': { bgcolor: 'rgba(0,224,255,0.1)' },
            mr: 1
          }}
        >
          👥 Contas
      ...
      {fichasFiltradas.convidados.length > 0 && (
        <Box sx={{ mb: 2 }}>
          <Typography variant="subtitle2" sx={{ color: '#ff9800', fontWeight: 'bold', borderBottom: '1px solid #ff9800', pb: 0.5, mb: 1 }}>🎭 CONVIDADOS ──</Typography>
          <List dense>
            {fichasFiltradas.convidados.map(fid => (
              <FichaItem
                key={fid}
                fid={fid}
                ficha={fichasDataMap[fid]}
      ...
      {/* 🟢 MODAL DE CONTAS */}
      <Dialog open={modalContasOpen} onClose={() => setModalContasOpen(false)} maxWidth="md" fullWidth>
        <DialogTitle sx={{ bgcolor: '#1a1a2e', color: '#fff' }}>
          👥 Contas Criadas ({contasUsuarios.length})
        </DialogTitle>
        <DialogContent sx={{ bgcolor: '#1a1a2e', color: '#fff', pt: 2 }}>
          {contasUsuarios.length === 0 ? (
            <Typography sx={{ color: '#64748b', textAlign: 'center', py: 4 }}>
