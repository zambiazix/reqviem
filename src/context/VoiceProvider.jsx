...
export default function VoiceProvider({ children }) {
  ...
  function requestServerNegotiationLock() {
    return new Promise((res) => {
      try {
        ...
        const onGrant = (p) => {
          if (p && p.grantedTo === socket.id) {
            cleanup();
            res({ granted: true, cbId });
          }
        };
        const cleanup = () => {
          try { socket.off("voice-negotiate-response", onResponse); } catch (e) {}
          try { socket.off("voice-negotiate-grant", onGrant); } catch (e) {}
          clearTimeout(timer);
        };
        socket.on("voice-negotiate-response", onResponse);
        socket.on("voice-negotiate-grant", onGrant);
  ...
  useEffect(() => {
        ...
        if (data.sdp) {
          ...
          try {
            ...
            if (isOffer) {
              try {
                const lockRes = await requestServerNegotiationLock();
                gotLock = !!(lockRes && lockRes.granted);
                if (gotLock) {
                  pendingNegotiations.current.add(from);
                } else {
                  // fallback: lock not granted -> we'll proceed anyway (avoid deadlock)
                  console.debug("voice-signal: lock not granted, proceeding to setRemoteDescription as fallback for", from);
                }
              } catch (e) {
                console.debug("requestServerNegotiationLock erro (proceeding fallback):", e);
              }
            }
    ...
    return () => {
      ...
      // Note: requestServerNegotiationLock has its own cleanup for its listeners.
    };
  }, [userNick, inVoice]);
